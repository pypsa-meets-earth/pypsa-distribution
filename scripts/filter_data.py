...
def rename_microgrid_buses(n, microgrids_list, output_path):
    ...
    for comp in ["lines", "links", "transformers", "loads", "generators", "stores", "storage_units"]:
        if hasattr(n, comp):
            df = getattr(n, comp)
...
def find_external_connection_buses(n, lines_path, shape_path, crs="EPSG:4326"):
    ...
    outside_raw_lines["start"] = outside_raw_lines.geometry.apply(lambda g: Point(g.coords[0]))
    outside_raw_lines["end"] = outside_raw_lines.geometry.apply(lambda g: Point(g.coords[-1]))
    # Identify which endpoint is inside the area
    outside_raw_lines["start_inside"] = outside_raw_lines["start"].apply(lambda p: area.contains(p))
    outside_raw_lines["end_inside"] = outside_raw_lines["end"].apply(lambda p: area.contains(p))
...
if __name__ == "__main__":
    ...
    mark_external_buses(
        n, 
        snakemake.input["raw_lines"], 
        snakemake.input["shape"], 
        output)